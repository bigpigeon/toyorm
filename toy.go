...
func Open(driverName, dataSourceName string) (*Toy, error) {
	...
	return &Toy{
		...
		DefaultHandlerChain: map[string]HandlersChain{
			...
			"HardDelete":               {HandlerHardDelete},
			"SoftDelete":               {HandlerSoftDelete},
			"HardDeleteWithPrimaryKey": {HandlerPreloadDelete, HandlerHardDelete},
			"SoftDeleteWithPrimaryKey": {HandlerPreloadDelete, HandlerSoftDelete},
		},
		DefaultModelHandlerChain: map[reflect.Type]map[string]HandlersChain{},
		ToyKernel: ToyKernel{
			Dialect: dialect,
